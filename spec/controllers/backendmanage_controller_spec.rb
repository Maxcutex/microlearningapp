...
describe BackEndManageController do
  describe "Admin Manage Courses" do
    it 'loads the manage courses page for a logged in an admin' do
      ...
      roles1 = Role.create(:role_name => 'Administrator', :role_description => 'Administrator on the system')
      roles2 = Role.create(:role_name => 'Instructor', :role_description => 'Course Co-ordinator on the system')
      roles3 = Role.create(:role_name => 'Student', :role_description => 'Student assessing courses on the system')
      
      userole1 = UserRole.create(:user_id => user.id, :role_id => roles1.id, :is_active => true)
      visit '/login'
